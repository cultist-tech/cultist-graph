...
export function saveMarketRentConditions(rentId: string, accountId: string, obj: JSONValue): void {
    ...
    for (let i = 0; i < royaltyObj.entries.length; i++) {
        ...
        updateMarketRentStats(rentId, accountId, saleCondition);

        saleCondition.save();
    }
}

export function updateMarketRentStats(
    saleId: string,
    accountId: string,
    ...
    if (saleCondition.ftTokenId === "near") {
        ...
        if (
            BigInt.fromString(stats.marketRentNearFloor).gt(BigInt.fromString(saleCondition.price))
        ) {
